...
function ServiceUse() {
  ...
  useEffect(() => {
    const hasVisited = localStorage.getItem('hasVisitedServiceUse');
    if (!hasVisited) {
      setIsModalOpen(true);
    }
  }, []);
  ...
  const closeModal = () => {
    localStorage.setItem('hasVisitedServiceUse', 'true');
    setIsModalOpen(false);
  };

  const handleNext = () => {
    const lessonId = 's1-service-use';
    const wasAlreadyCompleted = isCompleted(lessonId);
    gainExp(20, wasAlreadyCompleted);
    incrementCompletionCounts();
    if (!wasAlreadyCompleted) {
      addActivity({ icon: '💡', title: '[학습] AI 서비스 활용법 읽기 완료', time: '방금 전' });
      ...
      {isModalOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            <img src="/logo.png" alt="PrompE 로고" className="modal-logo-img" />
      ...
      <div className="service-use-page">
        ...
        <main className="lesson-content">
          <div className="lesson-title-section">
            ...
            <p className="lesson-subtitle">일상에서 쉽게 사용할 수 있는 AI 서비스를 소개합니다</p>
          </div>
          ...
          <article className="lesson-article">
            ...
            <section className="content-section">
              ...
              <div className="tip-box">
                ...
                <div className="tip-item"><span className="tip-number">3</span><div className="tip-content"><h4>예시를 함께 제공하기</h4><p>원하는 결과의 예시를 함께 보여주면 AI가 더 정확하게 이해할 수 있어요.</p></div></div>
                <div className="tip-item"><span className="tip-number">4</span><div className="tip-content"><h4>피드백 주기</h4><p>AI의 답변이 마음에 들지 않으면 "더 쉽게 설명해줘" 같은 추가 요청을 해보세요.</p></div></div>
              </div>
            </section>
            {/* ====================================================== */}
            ...
            <section className="content-section highlight-section">
              ...
              <p className="section-text">이제 AI 서비스가 무엇인지 알게 되었어요! 다음 레슨에서는 직접 AI와 대화하면서 더 효과적으로 소통하는 방법을 배워볼 거예요.</p>
              <button className="next-button" onClick={handleNext}>돌아가기</button>
            </section>
          </article>
        </main>
      </div>
